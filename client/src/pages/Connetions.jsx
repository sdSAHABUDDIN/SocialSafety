...
import { User, UserCheck, UserPlus, UserRoundPen } from 'lucide-react';
const Connetions = () => {
  const [currentTab,setCurrentTab]=React.useState('Flollowers ');
    ...
    <div className='min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {dataArray.map((tab)=>(
            <button key={tab.label} className={`flex items-center px-3 py-1 text-sm rounded-md cursor-pointer transition-colors ${currentTab=== tab.label ? 'bg-white font-medium text-black':'text-gray-500 hover:text'}`} onClick={()=>setCurrentTab(tab.label)}>
        ...
        {/* Connections List */}
        <div className='flex fles-wrap gap-6 mt-6'>
