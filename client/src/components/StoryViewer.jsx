...
const StoryViewer = ({viewStory, setViewStory}) => {
  ...
  const renderContent=()=>{
    switch(viewStory.media_type){
      case 'image':
        return (<img src={viewStory.media_url} alt="story" className='max-w-full max-h-screen object-contain'/>)
      case 'video':
        return (<video onEnded={()=>setViewStory(null)} src={viewStory.media_url} className=' max-h-screen ' autoPlay  controls/>)
      case 'text':
        return (
  ...
  return (
    ...
    >
      ...
      <div className='absolute top-0 left-0 w-full h-1 bg-gray-700'>
        <div className='h-full bg-white transition-all duration-100 linear' style={{width: `${progess}%`}}>
