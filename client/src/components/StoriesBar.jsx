...
const StoriesBar = () => {
  const [stories,setStrories]=React.useState([]);
  const [showModal,setShowModal]=React.useState(false);
  const [viewStory,setViewStory]=React.useState(false);

  const fetchStories=async()=>{
    //fetch stories from backend
    setStrories(dummyStoriesData)
  }
  useEffect(()=>{
    fetchStories();
  },[]) 
          ...
          stories.map((story,index)=>(
            <div onClick={()=> setViewStory(story)} key={index} className={`relative rounded-lg shadow min-w-30 max-w-30 max-h-40 cursor-pointer hover:shadow-lg transition-all duration-200 bg-gradient-to-b from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95`}>
              ...
              <p className='text-white absolute bottom-1 right-2 z-10 text-xs'>{moment(story.createdAt).fromNow()}</p>
              {
                ...
                <img src={story.media_url} alt="" className='w-full h-full object-cover hover:scale-110 transition duration-500 opacity-70 hover:opacity-80'/>
                :
                <video src={story.media_url} className='h-full w-full object-cover hover:scale-110 transition duration-500 opacity-70 hover:opacity-80' autoPlay muted loop/>
              }
                </div>
                )
              }
